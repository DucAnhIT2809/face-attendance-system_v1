...
from face_pipeline.paths import MODEL_DIR
from face_pipeline.recognition.arcface_train import ArcFaceBackbone, build_transform, resolve_device
...
DEFAULT_CHECKPOINT_NAME = "arcface_resnet18.pth"
LEGACY_CHECKPOINT_NAME = "arcface_resnet18_augmented.pth"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default=str(MODEL_DIR / "arcface_runs" / DEFAULT_CHECKPOINT_NAME),
        help=(
            "Path to ArcFace checkpoint (.pth). Default matches train-full output; "
            "falls back to arcface_resnet18_augmented.pth if that legacy file exists."
        ),
    )
    parser.add_argument(
        "--embedding-db",
        type=str,
        default=str(MODEL_DIR / "arcface_runs" / "face_db.pt"),
    ...
    parser.add_argument("--pg-db", type=str, default=DEFAULT_PG_DBNAME)
    parser.add_argument("--pg-user", type=str, default=DEFAULT_PG_USER)
    parser.add_argument(
        "--pg-password",
        type=str,
        default=default_pg_password(),
        help="Mật khẩu PostgreSQL (mặc định: biến FACE_ATTENDANCE_PG_PASSWORD hoặc 1234).",
    )
...
def resolve_checkpoint_path(checkpoint_arg: str) -> Path:
    ...
    default_checkpoint = (MODEL_DIR / "arcface_runs" / DEFAULT_CHECKPOINT_NAME).resolve()
    legacy_checkpoint = (MODEL_DIR / "arcface_runs" / LEGACY_CHECKPOINT_NAME).resolve()
    if checkpoint_path == default_checkpoint and legacy_checkpoint.exists():
...
class AttendanceDB:
    """Ghi điểm danh ArcFace/YOLO — dùng bảng riêng để không đụng `attendance_records` schema khác của app khác."""

    TABLE_NAME = "yolo_face_attendance"
    ...
    def mark_present(self, person_name: str, person_id: int, score: float) -> bool:
        ...
        with self.conn.cursor() as cursor:
            cursor.execute(
                f"""
                INSERT INTO {table} (attendance_date, person_name, person_id, last_score)
                SELECT %s, %s, %s, %s
                WHERE NOT EXISTS (
                    SELECT 1 FROM {table} ar
                    WHERE ar.attendance_date = %s AND ar.person_name = %s
                )
                RETURNING id;
                """,
                (today, person_name, person_id, score, today, person_name),
            )
