...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=str, default=str(MODEL_DIR / "arcface_runs"))
...
def save_metadata(save_dir: Path, class_names: Sequence[str], args: argparse.Namespace) -> None:
    td = Path(args.training_dir).resolve()
    try:
        training_dir_meta = str(td.relative_to(REPO_ROOT))
    except ValueError:
        training_dir_meta = str(td)
    metadata = {
        "class_names": list(class_names),
        "training_dir": training_dir_meta,
        "image_size": args.image_size,
        "embedding_size": args.embedding_size,
        "margin": args.margin,
        "scale": args.scale,
